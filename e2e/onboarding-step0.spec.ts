...
test.describe('Onboarding Step 0 — Google hit path', () => {
  test('top-hit card renders all metadata; confirm advances to Step 1 with banner + prefilled name', async ({ page, context }) => {
    ...
    const card = page.getByTestId('step0-top-hit');
    await expect(card).toBeVisible();
    await expect(card).toContainText(/Rua das Flores 123, São Paulo/i);
    await expect(card).toContainText(/Italian/i);
    await expect(card).toContainText(/4\.6 \(412\)/);
    await expect(card).toContainText(/\+55 11 5555 1234/);
    await screenshot(page, 'step0-top-hit');

    await page.getByTestId('step0-confirm-cta').click();
  ...
  test('"Search again" hides the top-hit card and re-enables the search form', async ({ page, context }) => {
    await stubScrape(context, [SAMPLE_HIT]);
    await page.goto('/onboarding');
    await waitForStep0(page);
    ...
    await page.getByLabel(/City/i).fill('São Paulo');
    await page.getByRole('button', { name: /Search Google Maps/i }).click();
    await expect(page.getByTestId('step0-top-hit')).toBeVisible();

    await page.getByRole('button', { name: /Search again/i }).click();
    await expect(page.getByTestId('step0-top-hit')).toHaveCount(0);
