 ...
 * Supports EN, ES, and PT languages with variable interpolation.
 *
 * Usage:
...
const TEMPLATES = {
  reservation_confirmed: {
    ...
    pt: ({ name, restaurant, date, time, partySize, reservationId }) =>
      `Oi ${name}, sua reserva no ${restaurant} esta confirmada!\n\n` +
      `Data: ${date}\n` +
      `Horário: ${time}\n` +
      `Pessoas: ${partySize}\n` +
      `Confirmacao: ${reservationId}\n\n` +
  ...
  reservation_cancelled: {
    ...
    pt: ({ name, reservationId, restaurant }) =>
      ...
      `Esperamos te ver em breve! Responda RESERVAR para fazer uma nova reserva.`,
  },
  ...
  reservation_reminder: {
    ...
    pt: ({ name, restaurant, date, time, partySize }) =>
      `Oi ${name}, estamos lembrando da sua reserva no ${restaurant}.\n\n` +
      `Data: ${date}\n` +
      `Horario: ${time}\n` +
      `Pessoas: ${partySize}\n\n` +
      `Responda CANCELAR se nao puder comparecer.`,
  },
  ...
  welcome: {
    ...
    pt: ({ restaurant }) =>
      `Bem-vindo ao ${restaurant}! Sou seu assistente de reservas.\n\n` +
      `Posso te ajudar a:\n` +
      `- Reservar uma mesa\n` +
  ...
  waitlist_added: {
    ...
    pt: ({ name, restaurant, partySize, estimatedWait }) =>
      `Oi ${name}, voce foi adicionado a lista de espera no ${restaurant}.\n\n` +
      `Pessoas: ${partySize}\n` +
      `Espera estimada: ${estimatedWait} minutos\n\n` +
  ...
  table_ready: {
    ...
    pt: ({ name, restaurant }) =>
      `Oi ${name}, otimas noticias! Sua mesa no ${restaurant} esta pronta.\n\n` +
...
function getTemplate(templateName, lang = 'en', vars = {}) {
  ...
  // Normalize pt-BR and pt-PT to pt
  const normalizedLang = lang && lang.startsWith('pt') ? 'pt' : lang;
