 ...
 *
 * Sends conversion emails to demo users at day 5 (2 days warning) and day 7 (last day).
 *
 * - Day-5 email: demos expiring in ~48h (expires between now+47h and now+49h)
 * - Day-7 email: demos expiring today (expires between now-1h and now+1h)
 *
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Day-5 window: demo expires between now+47h and now+49h
    ...
    // Day-7 window: demo expires between now-1h and now+1h
    ...
    const { data: day5Demos, error: day5Error } = await supabaseAdmin
      ...
      .select('id, restaurant_name, demo_token, demo_contact_email, demo_contact_name, demo_expires_at')
      .eq('is_demo', true)
    ...
    logger.info(`Found ${day5Demos?.length || 0} day-5 demos and ${day7Demos?.length || 0} day-7 demos`);
    ...
    for (const demo of (day5Demos || [])) {
      ...
      if (emailResult.success) {
        results.day5.sent++;
        // Mark as sent so we don't re-send on next run
        await supabaseAdmin
          .schema('restaurant')
          .from('restaurant_config')
    ...
    const totalFailed = results.day5.failed + results.day7.failed;
    if (totalFailed > 0) {
      captureMessage(
