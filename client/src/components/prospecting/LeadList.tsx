...
export function IntentPill({ intent }: { intent: string | null }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${INTENT_CLASS[intent] ?? 'bg-stone-100 text-stone-600'}`}>
    ...
    <GlassCard className="p-3 max-h-[70vh] overflow-y-auto">
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between gap-2">
              <span className={`truncate ${hot ? 'font-semibold' : 'font-medium'}`}>
              ...
              <span className="flex items-center gap-1.5 shrink-0">
                ...
                <span className="text-[11px] text-stone-400 w-8 text-right">{relTime(l.last_in_at || l.updated_at, nowMs)}</span>
              </span>
            </div>
            <div className="text-xs text-stone-500 truncate">
              {[l.city, l.sector, l.whatsapp_phone].filter(Boolean).join(' · ')}
              {snoozed && l.snoozed_until && (
                <span className="ml-1 text-stone-400">· adiado até {new Date(l.snoozed_until).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo', weekday: 'short', hour: '2-digit', minute: '2-digit' })}</span>
              )}
              {(l.touch_count ?? 1) > 1 && !l.last_in_at && (
                <span className="ml-1 text-stone-400">· toque {l.touch_count}/3</span>
              )}
            </div>
          </button>
        );
      ...
      {!loading && shown.length === 0 && (
        <p className="p-3 text-sm text-stone-500">
          {tab === 'triagem' ? 'Fila limpa — nada precisando de atenção. 🎉' : 'Nenhum lead.'}
        </p>
      )}
    </GlassCard>
  );
