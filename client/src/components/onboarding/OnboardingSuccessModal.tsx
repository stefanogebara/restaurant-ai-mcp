...
export default function OnboardingSuccessModal({ countdown, ownReferral, bookingUrl }: OnboardingSuccessModalProps) {
  ...
  // launch confirmation instead of staying on the (now-hidden) Step 6 form.
  useEffect(() => {
    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-6">
      ...
      >
        <div className="text-center">
          ...
          <h2 id="onboarding-success-title" className="font-serif text-3xl font-medium text-deep-charcoal mb-3">{t('onboarding.welcomeAboard')}</h2>
          ...
          {bookingUrl && (
            <div className="mb-6 border border-glass-border-dark rounded-2xl p-4 text-left">
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-soft-gray border border-glass-border-dark rounded-lg px-3 py-2 text-[12px] text-deep-charcoal font-mono truncate">
                  {bookingUrl}
                </div>
                <button
                  type="button"
          ...
          {ownReferral && (() => {
            ...
            // Share texts were hardcoded English — a PT-BR owner's WhatsApp
            // referral message went out in English (2026-06-10 audit).
            const whatsappText = encodeURIComponent(
              t('onboarding.referralShareWhatsapp', 'I just joined Seatable – the AI that manages restaurant reservations. Try it free: {{url}}', { url: referralUrl })
            );
            const emailSubject = encodeURIComponent(
              t('onboarding.referralShareEmailSubject', 'Try Seatable – AI reservations for restaurants')
            );
            const emailBody = encodeURIComponent(
              t('onboarding.referralShareEmailBody', 'Hey,\n\nI just started using Seatable – it handles restaurant reservations with AI. Thought you might find it useful.\n\nTry it free here: {{url}}\n\nCheers', { url: referralUrl })
            );
            return (
              <div className="mb-6 border border-glass-border-dark rounded-2xl p-4">
                ...
                <div className="flex gap-3 justify-center">
                  <a
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#9F1239] text-white text-[13px] font-medium hover:bg-burgundy-dark transition-colors"
                  >
