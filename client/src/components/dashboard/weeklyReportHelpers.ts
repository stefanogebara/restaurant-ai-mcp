...
export function buildDemoRows(demographics: WeeklyReportData['demographics'], t?: (key: string) => string) {
  ...
  return [
    { rank: 1, label: tr('analytics.tourists', 'Tourists'), detail: `${tourist_count} ${tr('analytics.visitors', 'visitors')}`, pct: tourist_percentage, count: null, color: colors.burgundy },
