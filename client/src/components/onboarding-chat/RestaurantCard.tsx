...
export default function RestaurantCard({ data, onEdit }: RestaurantCardProps) {
  return (
    <div className="w-full flex justify-start">
      <div
        className="max-w-[80%] bg-white border border-border-gray rounded-2xl rounded-bl-sm overflow-hidden"
      ...
      >
        <div className="p-4 space-y-2">
          ...
          {data.address && (
            <p className="text-xs text-muted-stone">📍 {data.address}</p>
          )}
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-stone">
            ...
            {typeof data.rating === 'number' && (
              <span>⭐ {data.rating} {data.review_count ? `(${data.review_count})` : null}</span>
            )}
        ...
        {onEdit && (
          <button
            type="button"
            onClick={onEdit}
            className="w-full text-left px-4 py-2 border-t border-border-gray text-xs text-burgundy hover:bg-warm-white transition-colors"
