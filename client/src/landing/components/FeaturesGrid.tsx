export default function FeaturesGrid() {
  ...
  return (
    <section id="features" className="py-24 px-6 sm:px-16 max-w-[1200px] mx-auto">
      {/* Header */}
      <div className="mb-16">
        <div className="text-xs font-semibold tracking-[2px] uppercase text-[#9F1239] mb-4">Features</div>
        <h2 className="font-serif text-4xl sm:text-[48px] font-medium leading-[1.15] tracking-tight text-[#1C1917] mb-4">
          Everything you need,<br />nothing you don&apos;t.
        </h2>
        <p className="text-[17px] text-[#78716C] font-light leading-[1.7] max-w-[480px]">
          A complete suite designed for modern restaurants that value simplicity and results.
        </p>
      </div>

      {/* 3x2 Grid */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-[2px] bg-[#E7E5E4] rounded-2xl overflow-hidden">
        {features.map((f, i) => (
          <div key={i} className="bg-white p-10 sm:p-12 hover:bg-[#FAFAF9] transition-colors">
            <div className="w-12 h-12 rounded-xl bg-[rgba(159,18,57,0.06)] flex items-center justify-center mb-6 text-xl text-[#9F1239]">
              {f.icon}
            </div>
            <h3 className="text-lg font-semibold text-[#1C1917] tracking-tight mb-3">{f.title}</h3>
            <p className="text-sm text-[#78716C] font-light leading-relaxed">{f.desc}</p>
          </div>
        ))}
      </div>
    </section>
