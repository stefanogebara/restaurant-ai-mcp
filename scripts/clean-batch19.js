...
const allFiles = [
  ...
  'scripts/restaurants-sp-batch13.json',
  'scripts/restaurants-sp-batch14.json',
  'scripts/restaurants-sp-batch15.json',
  'scripts/restaurants-sp-batch16.json',
  'scripts/restaurants-sp-batch17.json',
  'scripts/restaurants-sp-batch18.json',
];
...
const file = 'scripts/restaurants-sp-batch19.json';
...
const cleaned = list.filter(r => {
  ...
  if (!r.email) { console.log('SKIP (no email):', r.name); return false; }
  if (email.includes('@gmail.com') || email.includes('@yahoo.com') || email.includes('@hotmail.com')) {
    console.log('SKIP (personal email):', r.name, '—', r.email); return false;
  }
  if (sentEmails.has(email)) { console.log('SKIP (email dup):', r.name); return false; }
  if (sentNames.has(nameLower)) { console.log('SKIP (name dup):', r.name); return false; }
  if (skipFragments.some(f => nameLower.includes(f))) { console.log('SKIP (known contact):', r.name); return false; }
  if (domain && sentDomains.has(domain)) { console.log('SKIP (same domain/group):', r.name, '—', domain); return false; }
  return true;
});
