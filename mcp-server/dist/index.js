...
async function handleSeatParty(args) {
    ...
    const tableRecordIds = table_ids.map((tableNum) => {
        const table = tablesResult.data.records.find((t) => Number(t.fields.table_number) === Number(tableNum));
        if (!table) {
    ...
    const updatePromises = tableRecordIds.map((recordId) => airtable.updateRecord(TABLES_TABLE_ID, recordId, {
        'Status': 'Occupied',
...
async function handleCompleteService(args) {
    ...
    if (tableIds && Array.isArray(tableIds)) {
        const updatePromises = tableIds.map((tableRecordId) => airtable.updateRecord(TABLES_TABLE_ID, tableRecordId, {
