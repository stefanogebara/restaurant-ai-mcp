...
class AirtableClient {
  ...
  async getRecords(tableId: string, filter?: string) {
...
async function handleCheckAvailability(args: any) {
  ...
  const response = await axios.post(
    `${process.env.API_URL || 'https://restaurant-ai-mcp.vercel.app/api'}/check-availability`,
...
async function handleCreateReservation(args: any) {
  const {
    date,
    time,
    party_size,
...
async function handleLookupReservation(args: any) {
...
async function handleModifyReservation(args: any) {
  ...
  if (new_party_size) updates['Party Size'] = new_party_size;
...
async function handleCancelReservation(args: any) {
...
async function handleGetWaitTime(args: any) {
  ...
  if (activeParties > 8) waitTimeMinutes = 45;
  else if (activeParties > 5) waitTimeMinutes = 30;
  else if (activeParties > 3) waitTimeMinutes = 15;
...
async function handleGetHostDashboard(args: any) {
  // Get all tables
...
async function handleSeatParty(args: any) {
  const {
    type,
    reservation_id,
    customer_name,
...
async function handleCompleteService(args: any) {
...
async function handleMarkTableClean(args: any) {
  const { table_id } = args;
